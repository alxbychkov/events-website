...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_REQUEST["register_submit_button"] <> '' && !$USER->IsAuthorized())
{
	...
	if ($arResult["USE_CAPTCHA"] == "Y")
	{
		...
		$captcha = false;
		$SecretKey = $_REQUEST["recaptcha_token"];
		$response = getCaptcha($SecretKey, SECRET_KEY);
		if ($response && $response->success && $response->score > 0.5) $captcha = true;
		if (!$captcha) $arResult["ERRORS"]["CAPTCHAV3"] = 'Invalid capthcha';
	}
...
if($register_done)
{
	if(isset($_REQUEST["method"]) && isset($_REQUEST["event"])) {
		...
		echo '<script>if(document.querySelector(".registration__wrapper")) document.querySelector(".registration__wrapper").replaceWith("Вы зарегистрированы и успешно авторизовались."); add_to_data_profile("'.$method.'","'.$event.'").then(() => window.location.href = "'.$backurl.'");</script>';
	} else {
